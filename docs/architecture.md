...
- `internal/service`: domain business logic (audit lifecycle, run evaluation, issue creation).
...
3. Endpoint invokes service method.
4. Service validates inputs and executes business logic.
5. Repository persists/fetches records through Ent.
6. Transport encoder returns JSON response.
...
- `POST /audits/{id}/run` -> synchronous run + result evaluation + issue creation on deviation
