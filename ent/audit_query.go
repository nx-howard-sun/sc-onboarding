...
func (_q *AuditQuery) QueryRuns() *AuditRunQuery {
...
func (_q *AuditQuery) QueryIssues() *IssueQuery {
...
func (_q *AuditQuery) WithRuns(opts ...func(*AuditRunQuery)) *AuditQuery {
	query := (&AuditRunClient{config: _q.config}).Query()
...
func (_q *AuditQuery) WithIssues(opts ...func(*IssueQuery)) *AuditQuery {
	query := (&IssueClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *AuditQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Audit, error) {
	...
	if query := _q.withRuns; query != nil {
		if err := _q.loadRuns(ctx, query, nodes,
			func(n *Audit) { n.Edges.Runs = []*AuditRun{} },
			func(n *Audit, e *AuditRun) { n.Edges.Runs = append(n.Edges.Runs, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withIssues; query != nil {
		if err := _q.loadIssues(ctx, query, nodes,
			func(n *Audit) { n.Edges.Issues = []*Issue{} },
			func(n *Audit, e *Issue) { n.Edges.Issues = append(n.Edges.Issues, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *AuditQuery) loadRuns(ctx context.Context, query *AuditRunQuery, nodes []*Audit, init func(*Audit), assign func(*Audit, *AuditRun)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(auditrun.FieldAuditID)
	}
	query.Where(predicate.AuditRun(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(audit.RunsColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "audit_id" returned %v for node %v`, fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *AuditQuery) loadIssues(ctx context.Context, query *IssueQuery, nodes []*Audit, init func(*Audit), assign func(*Audit, *Issue)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*Audit)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(issue.FieldAuditID)
	}
	query.Where(predicate.Issue(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(audit.IssuesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
